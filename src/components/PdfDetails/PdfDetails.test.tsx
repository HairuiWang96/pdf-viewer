...
type PdfDetailsProps = React.ComponentProps<typeof PdfDetails>;

function renderDetails(props: Partial<PdfDetailsProps> = {}) {
  const onToggleStamp = vi.fn();
  const onClose = vi.fn();

  const merged = {
    metadata: makeCase(),
    // Required on this branch — the panel shows "Pages 1 / 5", so it needs to
    // know where the reader is, not just how long the document is.
    currentPage: 1,
    showStamp: false,
    onToggleStamp,
    isMobile: false,
