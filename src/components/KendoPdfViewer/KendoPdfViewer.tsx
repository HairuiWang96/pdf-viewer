...
import type { PDFViewerHandle, PDFViewerTool, PageEvent, ErrorEvent } from '@progress/kendo-react-all';
...
const DESKTOP_TOOLS: PDFViewerTool[] = ['pager', 'spacer', 'zoomInOut', 'zoom', 'selection', 'spacer', 'search', 'download', 'print'];
...
export default function KendoPdfViewer({ filePath, fileName, currentPage, onPageChange, onLoadSuccess, isMobile }: KendoPdfViewerProps) {
     ...
     *     viewer:           scrolls to page 3.                  wrong‼️
     *
    ...
    /**
     * ── Reporting the page count ───────────────────────────────────────────
     *
     * In one line: the viewer reporting back after opening a file — "done,
     * it's 5 pages" — and staying quiet if it has nothing true to say.
     *
     * Three similarly named things are chained here, which is the main thing
     * to keep straight. They are three links, not three versions of the same:
     *
     *   onLoad         Kendo's prop.  "Call me when you have read the file."
     ...
     *   onLoadSuccess(5)
     *        ↓
     *   the details panel can say "Page 3 of 5", the sidebar knows how many
     *   thumbnails to expect
     *
     * `pages` is Kendo's list of the pages it just read — a 5-page PDF gives
     * a list of 5. We never look inside them, only count them, which is why
     * the tests can pass [{}, {}] as a two-page document.
     *
     * The guard is the whole reason this is not a one-liner. A failed load
     * leaves `pages` empty, so the count is 0, and 0 is not an absence — it is
     * a claim:
     *
     *     "This document has 0 pages."   ← a lie the rest of the app believes
     *     say nothing                    ← the truth: we do not know yet
     *
     * Send 0 up and the details panel confidently prints "Page 1 of 0".
     */
