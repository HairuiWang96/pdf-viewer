...
async function generateAllPDFs() {
  for (const caseData of cases) {
    ...
    for (const page of stampedDoc.getPages()) {
      ...
      const textWidth = stampFont.widthOfTextAtSize(caseData.stampText, 10);

      page.drawText(caseData.stampText, {
    ...
    writeFileSync(`public/${caseData.fileName}-stamped.pdf`, stampedBytes);
    console.log(`Generated: public/${caseData.fileName}-stamped.pdf (${caseData.pages.length} pages, with stamp)`);
  }
}
